...
class DatabaseHelper {
  ...
  Future<void> insertTrxStatus(Map<String, dynamic> TrxStatus) async {
    final db = await database;
    await db.insert('TrxStatus', TrxStatus);
  }
  ...
  Future<List<Map<String, dynamic>>> getTrxStatus(String pumpNo) async {
    final db = await database; // Access the singleton database
    final List<Map<String, dynamic>> data = await db.query(
      'TrxStatus',
      where:
          'pumpNo = ?', // Find the record by transactionSeqNo (which corresponds to 'stan')
      whereArgs: [pumpNo],
      orderBy: 'id DESC',
    ); // Fetch data from fuel_sale table
  ...
  //return pending trxstatus

  Future<String?> getTrxStatuspending(String pumpNo) async {
    final db = await database; // Access the singleton database
    final List<Map<String, dynamic>> data = await db.query(
      'TrxStatus',
      where:
          'pumpNo = ? and status = ?', // Find the record by transactionSeqNo (which corresponds to 'stan')
      whereArgs: [pumpNo, "pending"],
    ); // Fetch data from fuel_sale table

    // Print all rows to the console (optional for debugging)
    data.forEach((row) {
      print("getTrxStatuspending: $row");
    });

    if (data.isNotEmpty) {
      print("getTrxStatuspending: ${data[0]['pos']}");
      return data[0]['pos'] as String?; // Return the 'pos' value
    } else {
      print("No pending transactions found for pumpNo: $pumpNo");
  ...
  //update status of trxstatus
  Future<void> updateTrxStatus(String transactionSeqNo) async {
    final db = await database;
    ...
    await db.update(
      'TrxStatus',
      {'status': "done"}, // Update the 'Stannumber' column
      where:
          'transactionSeqNo = ?', // Find the record by transactionSeqNo (which corresponds to 'stan')
      whereArgs: [
        transactionSeqNo
