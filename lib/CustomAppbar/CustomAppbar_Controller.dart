...
class CustomAppbarController extends GetxController {
  ...
  var payableTransaction = false.obs;
  ...
  Future<List<Map<String, dynamic>>> fetchTransactionsByshiftchecking(
      int shift) async {
    try {
      final dbHelper = DatabaseHelper();
      List<Map<String, dynamic>> data = await dbHelper.fetchDataByShift(shift);
  ...
  startConnection() async {
    ...
    if (await socketConnection.canConnect(5000, attempts: 3)) {
      ...
      Get.snackbar(
        'Alert'.tr,
        'Connection_Failed'.tr + ' , ' + 'Socket_not_connected'.tr,
        snackPosition: SnackPosition.TOP,
  ...
  saveTransaction(int paymantTypenum) async {
    ...
    try {
      ...
      payableTransaction.value = false;

      // dbHelper.updateTrxStatus(transactionSeqNo.value);
